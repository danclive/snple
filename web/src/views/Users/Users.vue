      ...
      <md-card>
        <md-card-header class="md-card-header-icon md-card-header-green">
          ...
          <div class="button-container" style="margin-top: 10px">
            <md-button class="md-success md-sm" @click="$router.push({ name: '添加用户' })">
...
export default {
  ...
  methods: {
    fecthDate() {
      userList({ page: this.pagination.currentPage, limit: this.pagination.perPage }).then(res => {
        this.tableData = res.data.items;
        this.pagination.total = res.data.count;
      });
    },
    handleEdit(item) {
      this.$router.push({ name: "用户详情", params: { id: item.id }});
    },
    handleDelete(item) {
    ...
    deleteUser() {
      ...
      userDelete(this.deleteId).then(res => {
        // console.log(res);
      });
      this.closeDeleteModal();
      this.fecthDate();
